...
# ── Config ───────────────────────────────────────────────────────────────────
REPO_URL="${REPO_URL:-https://github.com/YOUR_USER/second-brain.git}"
...
# ── Banner ───────────────────────────────────────────────────────────────────
echo ""
echo -e "${BOLD}  EM Second Brain — Installer${RESET}"
echo -e "${DIM}  Daily Datadog + GitHub dashboard, scheduled & emailed${RESET}"
divider
...
# Re-run install.sh to update, or edit this file directly.
...
DD_SITE=${DD_SITE}
DATADOG_DASHBOARD_URL_CATALOGUE_QUALITY=${DATADOG_DASHBOARD_URL_CATALOGUE_QUALITY}
DATADOG_DASHBOARD_URL_OWNER_METRICS=${DATADOG_DASHBOARD_URL_OWNER_METRICS}
DATADOG_TEAMS=${DATADOG_TEAMS}
...
# ── Todoist (todo list + reading queue) ──────────────────────────────────────
TODOIST_API_TOKEN=${TODOIST_API_TOKEN}
# TODOIST_PROJECT_ID is auto-populated by: python scripts/todo.py setup
EOF
...
#!/bin/zsh
# run-daily-dashboard.sh — generated by second-brain install.sh
...
echo "[\$(date)] Starting daily-dashboard run" >> "${LOG_FILE}"

agent -p --trust --force "\$(cat prompts/daily-dashboard.md)" >> "${LOG_FILE}" 2>&1

EXIT_CODE=\$?
echo "[\$(date)] Finished — exit code \$EXIT_CODE" >> "${LOG_FILE}"
exit \$EXIT_CODE
EOF
