...
post_install_guide() {
  ...
  echo ""
  echo -e "  ${BOLD}2) Agent CLI ${DIM}(optional for scheduled runs)${RESET}"
  if [[ -n "${SELECTED_AGENT:-}" ]]; then
    echo -e "     Selected: ${CYAN}${BOLD}${SELECTED_AGENT}${RESET} ($(agent_label "$SELECTED_AGENT"))"
  fi
  echo -e "     Scheduled runs use ${BOLD}scripts/run_daily_dashboard.py${RESET} by default."
  echo -e "     To run the skill through an agent, set ${BOLD}RUN_WITH_AGENT=1${RESET} and edit ${BOLD}AGENT_CLI${RESET}."
...
if [[ -n "$DETECTED_AGENTS" ]]; then
  success "Agent CLI(s) detected: $DETECTED_AGENTS (optional)"
else
  info "No agent CLI detected (scheduled runs use Python by default)"
fi
...
# Runs scripts/run_daily_dashboard.py by default; set RUN_WITH_AGENT=1 to invoke AGENT_CLI.
...
if [[ -f "\$LOG_FILE" ]]; then
  ...
  maxb=\$(( 50 * 1024 * 1024 ))
  if (( sz > maxb )); then
    printf '%s\n' "[\$(date)] Log cleared: previous size was \${sz} bytes (> 50 MiB)" >"\$LOG_FILE"
  fi
fi
...
set -o pipefail
run_dashboard() {
  if [[ "\${RUN_WITH_AGENT:-0}" == "1" ]]; then
    local prompt agent
    prompt=\$(cat "\$INSTALL_DIR/skills/engineering-pulse/SKILL.md")
    agent=\$(resolve_agent)
    run_agent "\$agent" "\$prompt" "\$INSTALL_DIR"
  else
    "\$INSTALL_DIR/.venv/bin/python" "\$INSTALL_DIR/scripts/run_daily_dashboard.py"
  fi
}

if [[ "\${RUN_WITH_AGENT:-0}" == "1" ]]; then
  RUN_LABEL="agent=\$(resolve_agent)"
else
  RUN_LABEL="python"
fi

if [[ -n \${ENGINEERING_PULSE_RUN_FG:-} ]]; then
  printf '%s\n' "[\$(date)] Starting daily-dashboard run (\$RUN_LABEL)" | tee -a "\$LOG_FILE"
  run_dashboard 2>&1 | tee -a "\$LOG_FILE"
  _ec=\$?
  printf '%s\n' "[\$(date)] Finished — exit code \$_ec" | tee -a "\$LOG_FILE"
else
  printf '%s\n' "[\$(date)] Starting daily-dashboard run (\$RUN_LABEL)" >> "\$LOG_FILE"
  if ! run_dashboard >>"\$LOG_FILE" 2>&1; then
    _ec=\$?
...
echo -e "  ${DIM}Run now:${RESET}      $RUNNER_SCRIPT"
